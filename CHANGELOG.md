...
### Added
- graphics: `renderer_mesh` uploads runtime geometry to device-local buffers
  without idling the graphics queue. The renderer retains fenced staging
  resources until each transfer completes, while preserving queue-ordered
  visibility for draws submitted behind it.
...
### Fixed
- window: Escape is delivered as ordinary application input instead of setting
  the platform close flag unconditionally. Window-manager close requests still
  stop the run loop, and applications choose whether Escape pauses, navigates,
  or calls `context_stop`.
...
- examples/shader: two cubes side by side, one through the built-in program and one through the game's own `Shader`.
...
### Changed
- shaders: follow mach 4.18.0. The SPIR-V types moved out of the compiler and into the target (mach#2888), so `sampler2d` and a hand-declared `#[spirv_op]` sampling instruction are gone; the fragment stages take `tex.Sampler2D` and `tex.sample_2d` from `shader.texture`, and mach-shader moves from `tag/0.1.0` to `branch/main`. `lit_frag`, `mesh_vert`, `ui_vert` and `ui_frag` are byte-identical across the change; only `skinned_vert` differs, and it is 988 bytes smaller.
...
### Removed
- graphics: the sprite program. 2D batching made it unreachable a release ago: a sprite has not been a draw of its own since, it is expanded into the frame's vertex arena and drawn through the immediate-mode program with everything else. The pipeline was still built and warmed at `renderer_init` and its unit quad still uploaded, for nothing. Gone with it: `PROGRAM_SPRITE`, `sprite_vert` and `sprite_frag` and their SPIR-V, `SpriteUniforms`, `src_uv_unit`, `vertex_format_sprite`, `SpriteResources`, and `set_layout_sprite`.

### Fixed
- graphics: boom requests **no optional device features at all**, where 0.3.1 required `shaderInt64` and `vertexPipelineStoresAndAtomics` and refused any device without them. Neither was asked for by a shader: the skinned stage's palette indices lowered to 64-bit locals and declared Int64 (mach#2878), and a storage buffer that is only read was not decorated `NonWritable`, so a driver had to assume the vertex stage might write it (mach#2879). Both are fixed in mach 4.18.0. Verified in the emitted module rather than from the release notes: `skinned_vert` declares `OpCapability Shader` alone and carries `OpDecorate NonWritable`, and `tools/check-uniform-layout.py` now fails on any capability past `Shader`.
- build: the shader build step copied `shaders/out/obj/shaders/*.spv` into `res/spv`. A glob takes whatever is in the directory, including the output of a shader that has since been deleted, so a removed module could be embedded again by the next build. The modules are named now.
